export const labelShape = {
  ...
  hitTest(shape, point) {
    ...
    const w = Math.max(8, shape.text.length * (textSize * 0.58));
    return point.x >= shape.x && point.x <= shape.x + w && point.y <= shape.y && point.y >= shape.y - textSize;
  },

  getBounds(shape) {
    const textSize = shape.style.textSize || 14;
    return { x: shape.x, y: shape.y - textSize, width: Math.max(8, shape.text.length * (textSize * 0.58)), height: textSize + 4 };
  },
